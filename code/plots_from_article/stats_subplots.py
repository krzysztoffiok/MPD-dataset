...
python3 stats_subplots.py --analyzed_set=mpd_lm

options for different metrics and parameter testing results have to be configured in lines after 205.

"""

parser = argparse.ArgumentParser(description='Classify data')

parser.add_argument('--analyzed_set', required=True, type=str, default="mpd_lm")
...
parser.add_argument('--no_trailing_zeros', required=False, default=True, action='store_true')
...
no_trailing_zeros = args.no_trailing_zeros
_fontsize = 20
...
trec_f_tests = KFData(file_list=[
                    ...
                    xlabel="TREC6 Learning rate parameters",
                    natat=False
                    )
...
selector = {"trec_lm": trec_lm, "mpd_lm": mpd_lm, "trec_tests": trec_f_tests, "mpd_tests": mpd_f_tests,
            "trec_mini_batch_size": trec_mini_batch_size,
            "trec_hidden_size": trec_hidden_size, "trec_shuffle": trec_shuffle}
analyzed_set = selector[analyzed_set]

if analyzed_set == trec_f_tests:
    param_list = [param_list[i] for i in [1, 4]]
    folder = "trec_learning_rate_parameters_data"
    analyzed_set.file_list = [join(f"./results/{folder}", x) for x in analyzed_set.file_list]
elif analyzed_set == mpd_f_tests:
    param_list = [param_list[i] for i in [1, 4]]
...
elif analyzed_set == trec_lm:
    param_list = [param_list[i] for i in [1, 3, 4]]
    folder = "paper_xlsx_trec"
    analyzed_set.file_list = [join(f"./results/{folder}", x) for x in analyzed_set.file_list]
elif analyzed_set == mpd_lm:
    param_list = [param_list[i] for i in [1, 3, 4]]
    folder = "paper_xlsx_mpd"
    analyzed_set.file_list = [join(f"./results/{folder}", x) for x in analyzed_set.file_list]
elif analyzed_set == trec_mini_batch_size:
    param_list = [param_list[i] for i in [1, 4]]
    folder = "trec_learning_rate_parameters_data"
    analyzed_set.file_list = [join(f"./results/{folder}", x) for x in analyzed_set.file_list]
elif analyzed_set == trec_hidden_size:
    param_list = [param_list[i] for i in [1, 3, 4]]
    folder = "trec_learning_rate_parameters_data"
    analyzed_set.file_list = [join(f"./results/{folder}", x) for x in analyzed_set.file_list]
elif analyzed_set == trec_shuffle:
    param_list = [param_list[i] for i in [1, 4]]
    folder = "trec_learning_rate_parameters_data"
    analyzed_set.file_list = [join(f"./results/{folder}", x) for x in analyzed_set.file_list]
...
if joint_plots:
    ...
    for num, param in enumerate(param_list[:]):
        ...
        elif analyzed_set == trec_hidden_size or analyzed_set == trec_mini_batch_size or analyzed_set == trec_shuffle:
            for numm, k in enumerate(df_temp.columns):
                xlabels.append(f"{k}")
        else:
            for numm, k in enumerate(df_temp.columns):
        ...
        for ii, v in enumerate(values):
            if not no_trailing_zeros:
                # remove leading 0
                if str(v).startswith("0"):
                    v = str(v)[1:]
            ax[num].text(ii + 1.0, float(v), f"{v:.3}", ha="center", fontsize=_fontsize, fontweight="bold")
        ...
        _title = _title.replace("MCC scores", "MCC Scores")
