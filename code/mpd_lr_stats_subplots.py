...
"""
Example messy code for generating plots from trained models. Needs improvement. Please contact if assistance needed.

Example use:

python3 mpd_lr_stats_subplots.py --analyzed_set=mpd_f_tests --symbolic --dataset=MPD
...
if analyzed_set == mpd_f_tests:
    ...
    analyzed_set.file_list = [join(f"./results/{dataset}/{folder}/{x[:-28]}", x) for x in analyzed_set.file_list]
elif analyzed_set == mpd_lm:
    param_list = [param_list[i] for i in [1, 3, 4]]
    folder = "paper_xlsx_mpd"
    analyzed_set.file_list = [join(f"./results/{dataset}/{folder}", x) for x in analyzed_set.file_list]
...
if joint_plots:
    ...
    for num, param in enumerate(param_list[:]):
        ...
        if not analyzed_set.natat:
            tukey_ = open(f"./results/{dataset}/statistics/TREC6_{param}_{analyzed_set.xlabel}.txt", "w+")
        else:
            tukey_ = open(f"./results/{dataset}/statistics/MPD_{param}_{analyzed_set.xlabel}.txt", "w+")
    ...
    plt.savefig(f"./results/{dataset}/images/{analyzed_set.xlabel}_{len(param_list)}.png")